  ...
  return (
    <div className="px-5 md:px-8 pt-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-t1 tracking-tight">Projects</h1>
        <div className="flex items-center gap-2">
          ...
          <motion.button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-s2 hover:bg-t1/[0.08] text-t1 text-sm font-medium rounded-xl transition-colors border border-t1/[0.06]"
          >
            + New Project
          </motion.button>
        </div>
        ...
        {groups.map((group, i) => {
          ...
          return (
            <motion.button
              ...
              className={`flex items-center justify-between px-4 py-4 rounded-2xl border text-left transition-all shadow-card ${
                active
                  ? 'bg-accent/10 border-accent/25'
                  : 'bg-surface border-t1/[0.06] hover:border-t1/[0.12] hover:shadow-card-md'
              }`}
            >
              <div className="flex items-center gap-3">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${active ? 'bg-accent/20' : bgClass}`}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} className={`w-4 h-4 ${active ? 'text-accent' : textClass}`}>
                ...
                <div>
                  ...
                  {group.is_default && (
                    <p className="text-xs text-t3 mt-0.5">Default</p>
                  )}
                </div>
              </div>
              <span className={`text-sm font-bold ${active ? 'text-accent' : 'text-t3'}`}>
                {count}
              </span>
            </motion.button>
          );
      ...
      <AnimatePresence>
        {selectedGroup && (
          ...
          >
            ...
            {tasks.length === 0 ? (
              <p className="text-t3 text-sm text-center py-8">No tasks in this project</p>
            ) : (
      ...
      <Modal open={newGroupOpen} onClose={() => setNewGroupOpen(false)} title="New Project">
